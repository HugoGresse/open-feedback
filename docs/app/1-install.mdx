...
the legacy `functions.config()` / Runtime Config is deprecated and is not read:
`firebase.json` sets `disallowLegacyRuntimeConfig`, so values set with
`firebase functions:config:set` are ignored). Add the
...
emulator only). These files hold secrets — the repo `.gitignore` ignores every
`functions/.env*` except the `.env.default` placeholder, so create them
locally and never commit real values. Example `functions/.env.local`:
...
And then create `functions/.env.<your-project-id>` yourself (the functions
read directly from `process.env`, so the Firebase CLI will not generate it for
you) and add the admin contact email:
