...
In the firebase console, create a new account with editor permissions and download the json key file.

Configure your app mode:

    firebase functions:config:set app.env="production" # or "development"

Copy the json key file in `functions/serviceAccountKey.production.json` (or serviceAccountKey.development.json)
