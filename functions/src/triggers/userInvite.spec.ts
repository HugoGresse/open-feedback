...
describe('userInviteCreated', () => {
    ...
    it('should reject when a user is invited to a project, email sent but db update failed', async () => {
        ...
        update.mockImplementation(() => Promise.reject('firestore update failed'))
        ...
        await expect(userInviteCreatedWrapped(snapshot)).rejects.toEqual('firestore update failed')

        expect((send as any).mock.calls[0][0]).toEqual({'api': 'MAILGUN_API', 'domain': 'MAILGUN_DOMAIN', 'key': 'MAILGUN_KEY'})
        expect((send as any).mock.calls[0][1].subject).toEqual('[OpenFeedback] Hugo G invited you to become member of the event "Project Name"')
        expect((send as any).mock.calls[0][1].to).toEqual(["email@example.com"])
        expect((send as any).mock.calls[0][1].html).toContain('<!DOCTYPE html>\n<html')
    ...
    it('should resolve when a user is invited to a project, thus an email is sent', async () => {
        const {update, get, collection, doc, where} = getFirestoreMocksAndInit()
        ...
        await expect(userInviteCreatedWrapped(snapshot)).resolves.toEqual('forEach not implemented or firebase issue')
        ...
        expect(where).toHaveBeenCalledWith('email', '==', invite.destinationUserInfo)
    })
})
...
describe('checkPendingInviteAndProcessThem', () => {
    ...
    it('check pending invites, no nothing is none was found', async () => {
        ...
        expect(collection).toHaveBeenCalledWith('projects-invites')
        expect(where).toHaveBeenCalledWith('destinationUserInfo', '==', user.email)
    ...
    it('check pending invites and assert process', async () => {
        ...
        expect(get, 'projects-invites has been queried').toHaveBeenCalledTimes(1)
        expect(update, 'projects and projects-invites has been updated once each for each invite').toHaveBeenCalledTimes(4)
    })
})
