...
export const getSessionsAsArray = createSelector(getSessionsList, sessions => {
...
export const getFilteredSessions = createSelector(getSessionsList, list => {
  return list
})

export const getSessionsGroupByDate = createSelector(getSessionsAsArray, getSessionsFilter, (sessions, filter) => {
  const sessionsGroupByDate = groupBy(sessions, session => moment(session.startTime).format('YYYY-MM-DD'))

  return Object.keys(sessionsGroupByDate).reduce((acc, date) => {
    acc.push({
      date: date,
      sessions: sessionsGroupByDate[date].filter(session => session.title.toLowerCase().includes(filter.toLowerCase()))
    })
    return acc
  }, [])
})
