...
const schema = object().shape({
    jsonUrl: string().url().required(
        <Typography>The JSON URL is required</Typography>
    )
})

const Step3JSON = ({onCancel, onSubmit, stepTitle, submitText}) => {
            ...
            {({isSubmitting}) => (
                <Form method="POST">
                    ...
                    <div className={classes.jsonExample}>
                        <Button className={classes.jsonShowButton}
                                onClick={() => setExampleOpen(!isExampleOpen)}>Show JSON model <ArrowDownIcon/></Button>
