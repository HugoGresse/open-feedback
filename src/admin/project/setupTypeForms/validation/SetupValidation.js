    ...
    return <Box>
        ...
        <Grow in={connectionState !== STATE_IDLE}>
            <Box bgcolor="#eee" borderRadius={5} padding={2} marginTop={2}>
                ...
                <Check state={modelState} text={modelText} subText={modelSubtitleText}/>
            </Box>
        </Grow>
    </Box>
...
const Check = ({state, text, subText}) => {
    ...
    switch (state) {
        ...
        case STATE_IDLE:
            ...
            break
        case STATE_LOADING:
            stateIcon = <CircularProgress style={{width: 20, height: 20, color: "#111", marginRight: 14}}/>
            break
        case STATE_SUCCESS:
            stateIcon = <CheckIcon style={{color: "#00cc00", marginRight: 10}}/>
            break
        case STATE_ERROR:
    ...
    return <Box display="flex" marginBottom={1}>
        {stateIcon}
        <Box>
            <Typography>
                {text}
